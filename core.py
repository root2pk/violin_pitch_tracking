"""
Core functions for pitch estimation with the CREPE model, cloned from the original repository at https://github.com/marl/crepe
Viterbi methods have been modified.
"""
...
def to_weird_viterbi_cents(salience):
    ...
    Parameters:
    salience : np.ndarray
        The salience/activation matrix

    Returns:
    path : np.ndarray
        The Viterbi path
    cents : np.ndarray
        The estimated pitch in cents
    """
    ...
    cents = np.array([to_local_average_cents(salience[i, :], path[i]) for i in range(len(path))])
